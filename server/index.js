...
const filePath = path.join(__dirname, 'scraped_github_data.json');
...
app.get('/scrape_multiple_github', async (req, res) => {
  ...
  const allUserData = [...existingData];
  ...
  for (const entry of githubData) {
    ...
    const isExisting = allUserData.some(user => user.resident === username);
    ...
    try {
      ...
      console.error(`Failed to scrape data for ${username}:`, error);
    }
  }
